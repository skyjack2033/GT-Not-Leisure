...
public class ElytraBoostPacket implements IMessage, IMessageHandler<ElytraBoostPacket, IMessage> {
    ...
    public IMessage onMessage(ElytraBoostPacket message, MessageContext ctx) {
        ...
        if (!InfinityElytra.isWearingInfinityElytra(player)) return null;
        if (!(player instanceof IElytraPlayer)) return null;
        if (!((IElytraPlayer) player).etfu$isElytraFlying()) return null;

        NBTTagCompound data = player.getEntityData();
        int lastBoost = data.getInteger("InfinityElytraLastBoost");
        if (player.ticksExisted - lastBoost < COOLDOWN_TICKS) return null;
