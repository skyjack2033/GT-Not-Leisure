...
public class DetravMapTexture extends AbstractTexture {
    ...
    public final ProspectingPacket.ProspectingWrapper packet;
    private String selected = "All";
    public int width = -1;
    public int height = -1;
    public boolean invert = false;

    public DetravMapTexture(ProspectingPacket.ProspectingWrapper aPacket) {
        packet = aPacket;
    }
                ...
                if (packet.map()[i][j] != null) {
                    if (packet.ptype() == 0 || packet.ptype() == 1) {
                        for (short meta : packet.map()[i][j].values()) {
                            final String name = packet.metaMap()
                                .get(meta);
                            ...
                            image.setRGB(
                                ...
                                j,
                                packet.ores()
                                    .getOrDefault(name, Color.BLACK.getRGB()) | 0XFF000000);
                            break;
                        }
    ...
    public void loadTexture(IResourceManager resourceManager) {
        ...
        if (packet != null) {
            ...
            width = (packet.size() * 2 + 1) * 16;
            height = (packet.size() * 2 + 1) * 16;
        }
    }
